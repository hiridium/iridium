...
def get_options():
    mc_path_default = str(Path.home() / "iridium")
    ...
    parser.add_argument("-m", "--iridium", metavar="DIR", default=mc_path_default,
                        help="Iridium path prefix (default: %(default)s)")
    ...
    if not Path(options.iridium).exists():
        parser.error("{!r}: Iridium path does not exist".format(options.iridium))

    logger.info("{} - {}".format(logger.name, parser.description))
    logger.info("  Path:     {}".format(options.iridium))
...
def main():
    ...
    options = get_options()
    os.chdir(str(Path(options.iridium) / "v8build" / "v8" / "out.gn" / "x64.release"))
