                            ...
                            case MC_PRM_STRING  : 
                                ...
                                if((BitchainParamArray+i)->m_Type & MC_PRM_SPECIAL)
                                {
                                    if(strcmp(param->m_Name,"chaindescription") == 0)
                                    {
                                        ...
                                        {
                                            sprintf(ptrData,"Iridium %s",name);
                                        }
...
int mc_BitchainParams::Validate()
{
    ...
    if(isValid)
    {
        ...
        else
        {
            ...
            if(strcmp((char*)protocol_name,"iridium") == 0)
            {
...
int mc_BitchainParams::Print(FILE* fileHan)
{
    ...
    fprintf(fileHan,"# ==== Iridium configuration file ====\n\n");
    fprintf(fileHan,"# Created by iridium-util \n");
    ...
    switch(m_Status)
    {
        case MC_PRM_STATUS_EMPTY:
            ...
            fprintf(fileHan,"# To join network please run \"iridiumd %s@<seed-node-ip>[:<seed-node-port>]\".\n",Name());
            return MC_ERR_NOERROR;
        case MC_PRM_STATUS_ERROR:
            ...
            fprintf(fileHan,"# Please fix it and rerun iridium-util. \n");
            break;
        ...
        case MC_PRM_STATUS_GENERATED:
            ...
            fprintf(fileHan,"# To generate network please run \"iridiumd %s\".\n",Name());
            break;
        case MC_PRM_STATUS_VALID:
            fprintf(fileHan,"# This parameter set is VALID. \n");
            fprintf(fileHan,"# To join network please run \"iridiumd %s\".\n",Name());
            break;
    }
        ...
        while(i>=0)
        {
                    ...
                    ((m_lpParams+i)->IsRelevant(version) > 0))
            {
                ...
                if(header_printed == 0)
                {
                    ...
                    {
                        case MC_PRM_COMMENT:
                            fprintf(fileHan,"# The following parameters don't influence iridium network configuration. \n");
                        ...
                        case MC_PRM_USER:
                            if(m_Status == MC_PRM_STATUS_ERROR)
                            {
                                ...
                                if(Name())
                                {
                                    fprintf(fileHan,"# Please rerun \"iridium-util clone %s <new-network-name>\". \n",Name());
                                }
                            }
                            else
                            {
                                ...
                                {
                                    fprintf(fileHan,"# The following parameters can be edited before running iridiumd for this chain. \n");                                    
                                }
                                else
                                {
                                    fprintf(fileHan,"# The following parameters can only be edited if this file is a prototype of another configuration file. \n");
                                    fprintf(fileHan,"# Please run \"iridium-util clone %s <new-network-name>\" to generate new network. \n",Name());
                                }
                            }
                            break;
                        case MC_PRM_GENERATED:
                            fprintf(fileHan,"# The following parameters were generated by iridium-util.\n");
                            fprintf(fileHan,"# They SHOULD ONLY BE EDITED IF YOU KNOW WHAT YOU ARE DOING. \n");
                            break;
                        case MC_PRM_CALCULATED:
                            fprintf(fileHan,"# The following parameters were generated by iridiumd.\n");
