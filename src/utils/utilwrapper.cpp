...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Iridium
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Iridium
    // Mac and Unix: ~/.iridium
#ifdef WIN32
    // Windows
    if(mc_gState->m_SessionFlags & MC_SSF_COLD)
    {
        return GetSpecialFolderPath(CSIDL_APPDATA) / "IridiumCold";
    }
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Iridium";
#else
    // Mac and Unix
    ...
    if(mc_gState->m_SessionFlags & MC_SSF_COLD)
    {
        return pathRet / ".iridium-cold";        
    }
    return pathRet / ".iridium";
#endif
}
...
void mc_CheckDataDirInConfFile()
{
    ...
    mapConfig=new mc_MapStringString;
    if(mc_ReadGeneralConfigFile(mapConfig,NULL,"iridium",".conf") == 0)
    {
...
int mc_Params::ReadConfig(const char *network_name)
{
    ...
    mc_ReadConfigFile(mc_GetConfigFile(network_name,"iridium",".conf"),&mapArgs, &mapMultiArgs,"-");    
