...
#include "iridium/iridium.h"                                              // MCHN
...
/* MCHN START */
static boost::filesystem::path pathCachedIridium;
static boost::filesystem::path pathCachedIridiumLog;
...
/* MCHN START */
    ...
    pathCachedIridium=fs::path(string(mc_gState->m_Params->DataDir()));
    fs::path &path =pathCachedIridium;
    return path;
...
/* MCHN START */
    fs::path &path =pathCachedIridiumLog;
    if (!path.empty())
        return path;
    path=fs::path(string(mc_gState->m_Params->DataDir(2,1)));
    return pathCachedIridiumLog;
}
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "iridium.pid"));
