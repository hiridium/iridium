...
int mc_IridiumFilterParams::Zero()
{
    m_MaxShownData=-1;
...
int mc_IridiumFilterParams::Init()
{
    Zero();
...
int mc_IridiumFilterParams::Close()
{
    mc_gState->m_Compatibility=m_Compatibility;    
    return MC_ERR_NOERROR;
}

int mc_IridiumFilterParams::Destroy()
{
    return Zero();
}
...
int mc_IridiumLibrary::Initialize(const unsigned char* short_txid,uint32_t index)
{
    return MC_ERR_NOERROR;    
}

int mc_IridiumFilter::Zero()
{
...
int mc_IridiumFilter::Initialize(const unsigned char* short_txid)
{
    size_t value_size;
    unsigned char *ptr;
    ...
    if(ptr)
    {
        m_FilterCodeRow=pIridiumFilterEngine->m_CodeLibrary->GetNumElements();
        pIridiumFilterEngine->m_CodeLibrary->AddElement();
        pIridiumFilterEngine->m_CodeLibrary->SetData(ptr,value_size);
        pIridiumFilterEngine->m_CodeLibrary->SetData(&ntc,1);        
    }                                    
    else
    {
        m_FilterCodeRow=pIridiumFilterEngine->m_CodeLibrary->GetNumElements();
...
bool mc_IridiumFilter::HasRelevantEntity(set <uint160>& sRelevantEntities)
{
...
int mc_IridiumFilterEngine::Zero()
{
    m_Filters.clear();
    m_TxID=0;
    m_EntityTxID=0;
...
int mc_IridiumFilterEngine::Destroy()
{
    if(m_Semaphore)
    {
...
void mc_IridiumFilterEngine::Lock(int write_mode)
{        
...
void mc_IridiumFilterEngine::UnLock()
{    
    m_LockedBy=0;
    __US_SemPost(m_Semaphore);
}

int mc_IridiumFilterEngine::InFilter()
{    
...
int mc_IridiumFilterEngine::GetAcceptTimeout()
{    
    return GetArg("-acceptfiltertimeout",DEFAULT_ACCEPT_FILTER_TIMEOUT);
}

int mc_IridiumFilterEngine::GetSendTimeout()
{    
...
int mc_IridiumFilterEngine::SetTimeout(int timeout)
{
    Lock(1);
...
int mc_IridiumFilterEngine::SetTimeoutInternal(int timeout)
{
    for(int i=0;i<(int)m_Filters.size();i++)
    {
        mc_Filter *worker=*(mc_Filter **)m_Workers->GetRow(i);
...
uint160 mc_IridiumFilterEngine::ActiveUpdateID(uint160 hash)
{
    map<uint160,mc_IridiumLibrary>::iterator it=m_Libraries.find(hash);
...
int mc_IridiumFilterEngine::LoadLibrary(uint160 hash,bool *modified)
{
    int count;
    size_t value_size;
    unsigned char *ptr;
    ...
    for(int i=0;i<count;i++)
    {
        ...
        map<uint160,mc_IridiumLibrary>::iterator it=m_Libraries.find(library[i].m_Hash);
        if (it == m_Libraries.end())
        {
            if(i == 0)
            {
...
int mc_IridiumFilterEngine::CheckLibraries(set<uint160>* lpAffectedLibraries, int for_block)
{
    Lock(1);
...
int mc_IridiumFilterEngine::CheckLibrariesInternal(set<uint160>* lpAffectedLibraries, int for_block)
{
    int err;
    bool modified;
    map<uint160,mc_IridiumLibrary>::iterator mit;
    map<uint160,mc_IridiumLibrary>::iterator lit;    
...
mc_Filter *mc_IridiumFilterEngine::StreamFilterWorker(int row,bool *modified)
{
    int err; 
    char *code;
    size_t code_size;
    ...
    *modified=false;
    ...
    for (unsigned int i=0;i<m_Filters[row].m_Libraries.size();i++) 
    {
        ...
        map<uint160,mc_IridiumLibrary>::iterator it=m_Libraries.find(mc_LibraryIDForUpdate(m_Filters[row].m_Libraries[i]));
        if(it == m_Libraries.end())
        {
...
int mc_IridiumFilterEngine::RebuildFilter(int row,int for_block)
{
    int err; 
    ...
    for (unsigned int i=0;i<m_Filters[row].m_Libraries.size();i++) 
    {
        map<uint160,mc_IridiumLibrary>::iterator it=m_Libraries.find(m_Filters[row].m_Libraries[i]);
        if (it != m_Libraries.end())
        {
...
int mc_IridiumFilterEngine::AddFilter(const unsigned char* short_txid,int for_block)
{
    Lock(1);
...
int mc_IridiumFilterEngine::AddFilterInternal(const unsigned char* short_txid,int for_block)
{    
    int err;
    mc_IridiumFilter filter;
...
int mc_IridiumFilterEngine::Reset(int block,int for_block)
{
    Lock(1);
...
int mc_IridiumFilterEngine::ResetInternal(int block,int for_block)
{
    int filter_block;
    int err;
...
int mc_IridiumFilterEngine::NoStreamFilters()
{
    int ret=1;
...
int mc_IridiumFilterEngine::RunStreamFilters(const CTransaction& tx,int vout, unsigned char *stream_short_txid,int block,int offset,std::string &strResult,mc_IridiumFilter **lppFilter,int *applied)            
{
    if(mc_gState->m_Features->StreamFilters() == 0)
    {
        return MC_ERR_NOERROR;
...
int mc_IridiumFilterEngine::RunTxFilters(const CTransaction& tx,std::set <uint160>& sRelevantEntities,std::string &strResult,mc_IridiumFilter **lppFilter,int *applied,bool only_once)
{    
    Lock(0);
...
int mc_IridiumFilterEngine::RunFilter(const CTransaction& tx,mc_Filter *filter,std::string &strResult)
{
    Lock(0);
    int err=MC_ERR_NOERROR;
    m_Tx=tx;
...
int mc_IridiumFilterEngine::RunFilterWithCallbackLog(const CTransaction& tx,int vout,uint256 stream_txid,mc_Filter *filter,std::string &strResult, json_spirit::Array& callbacks)
{
    Lock(0);
    int err=MC_ERR_NOERROR;
    m_Tx=tx;
...
void mc_IridiumFilterEngine::SetCallbackNames()
{
    Lock(1);
...
void mc_IridiumFilterEngine::SetCallbackNamesInternal()
{
    m_CallbackNames.clear();
...
int mc_IridiumFilterEngine::Initialize()
{
    mc_Buffer *filters;
    unsigned char *txid;
    int err=MC_ERR_NOERROR;
