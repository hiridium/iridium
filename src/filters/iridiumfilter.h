...
#ifndef IRIDIUMFILTER_H
#define IRIDIUMFILTER_H
...
#define MC_FLT_MAIN_NAME_TEST              "_iridium_library_test_"
...
typedef struct mc_IridiumFilter
{
...
typedef struct mc_IridiumLibrary
{
    mc_EntityDetails m_Details;
    std::string m_CreateError;
...
typedef struct mc_IridiumFilterParams
{
    int m_MaxShownData;
    int m_Compatibility;
    mc_IridiumFilterParams()
    {
        Zero();
    }
...
typedef struct mc_IridiumFilterEngine
{
    std::vector <mc_IridiumFilter> m_Filters;
    std::vector <std::vector <std::string>> m_CallbackNames;
    std::map<uint160,mc_IridiumLibrary> m_Libraries;
    ...
    CTransaction m_Tx;
    int m_Vout;
    mc_IridiumFilterParams m_Params;
    void *m_CoinsCache;
    
    void *m_Semaphore;
    uint64_t m_LockedBy;
    
    mc_IridiumFilterEngine()
    {
        Zero();
    }
    ...
    int RunTxFilters(const CTransaction& tx,std::set <uint160>& sRelevantEntities,std::string &strResult,mc_IridiumFilter **lppFilter,int *applied,bool only_once);            
    int RunStreamFilters(const CTransaction& tx,int vout, unsigned char *stream_short_txid,int block,int offset,std::string &strResult,mc_IridiumFilter **lppFilter,int *applied);            
