...
#include "filters/iridiumfilter.h"

extern mc_IridiumFilterEngine* pIridiumFilterEngine;
...
typedef struct CIridiumTxDetails
{    
...
bool CIridiumTxDetails::IsRelevantInput(int vin, int vout)
{
...
bool CIridiumTxDetails::IsRelevantEntity(uint160 hash)
{
...
void CIridiumTxDetails::SetRelevantEntity(void *entity)
{
    uint160 hash=0;
...
void CIridiumTxDetails::SetAffectedLibrary(void *entity)
{
    uint160 hash=0;
    memcpy(&hash,entity,MC_AST_SHORT_TXID_SIZE);
...
bool mc_CompareAssetQuantities(CIridiumTxDetails *details,string& reason)
{
    unsigned char *ptrIn;
...
void IridiumTransaction_SetTmpOutputScript(const CScript& script1)
{
    CScript::const_iterator pc1 = script1.begin();
    mc_gState->m_TmpScript->Clear();
    mc_gState->m_TmpScript->SetScript((unsigned char*)(&pc1[0]),(size_t)(script1.end()-pc1),MC_SCR_TYPE_SCRIPTPUBKEY);
...
bool IridiumTransaction_CheckCachedScriptFlag(const CTransaction& tx)
{
    bool flag=false;
...
bool IridiumTransaction_CheckCoinbaseInputs(const CTransaction& tx,     
                                               CIridiumTxDetails *details)
{
    if(!tx.IsCoinBase())                                                  
    {   
        return false;
...
bool IridiumTransaction_CheckInputs(const CTransaction& tx,                  // Tx to check
                                       const CCoinsViewCache &inputs,           // Tx inputs from UTXO database
                                       CIridiumTxDetails *details,           // Tx details object
...
bool IridiumTransaction_CheckNewEntity(int vout,
                                          bool& fScriptParsed,
                                          CIridiumTxDetails *details,   
                                          string& reason)      
{
...
bool IridiumTransaction_CheckCachedScript(const CTransaction& tx, 
...
void IridiumTransaction_FillAdminPermissionsBeforeTx(const CTransaction& tx,
                                                        CIridiumTxDetails *details)      
{
...
bool IridiumTransaction_VerifyAndDeleteDataFormatElements(string& reason,int64_t *total_size,uint32_t *salt_size)
{    
...
bool IridiumTransaction_CheckOpReturnScript(const CTransaction& tx, 
                                               const CCoinsViewCache &inputs,   
                                               int vout,
                                               CIridiumTxDetails *details,   
                                               string& reason)      
{
    ...
    total_offchain_size=0;
    if(!IridiumTransaction_VerifyAndDeleteDataFormatElements(reason,&total_offchain_size,NULL))
    {
        return false;
    }
    ...
    if( mc_gState->m_TmpScript->GetNumElements() == 2 )                         // Cached input script or new entity
    {
        ...
        {
                                                                                // Cached scripts
            if(!IridiumTransaction_CheckCachedScript(tx, inputs, fScriptParsed, details, reason))
            {
                return false;
            }
        }
        ...
        {
                                                                                // New entities
            if(!IridiumTransaction_CheckNewEntity(vout, fScriptParsed, details, reason))
            {
                return false;
            }
        }
                                                                                ...
                                                                                // Performed only if it is indeed needed
    {
        ...
        {
            IridiumTransaction_FillAdminPermissionsBeforeTx(tx,details);
        }                        
    }
...
bool IridiumTransaction_CheckAssetUpdateDetails(mc_EntityDetails *entity,
                                                   int vout,
                                                   CIridiumTxDetails *details,   
                                                   string& reason)      
{
...
bool IridiumTransaction_CheckVariableUpdateDetails(const CTransaction& tx,
                                                   mc_EntityDetails *entity,
                                                   int offset, 
                                                   int vout,
                                                   CIridiumTxDetails *details,   
...
bool IridiumTransaction_CheckUpgradeApproval(const CTransaction& tx,
                                                mc_EntityDetails *entity,
                                                int offset,  
                                                int vout,
                                                CIridiumTxDetails *details,   
...
bool IridiumTransaction_CheckStreamItem(mc_EntityDetails *entity,
                                           int vout,
                                           CIridiumTxDetails *details,   
                                           string& reason)      
{
...
bool IridiumTransaction_CheckEntityItem(const CTransaction& tx,
                                           int offset,  
                                           int vout,
                                           CIridiumTxDetails *details,   
                                           string& reason)      
{
    ...
    if(!IridiumTransaction_VerifyAndDeleteDataFormatElements(reason,NULL,&salt_size))
    {
        return false;
    }
       ...
       (entity.GetEntityType() == MC_ENT_TYPE_LIBRARY))                         // Asset, variable or library update
    {
        if(entity.GetEntityType() == MC_ENT_TYPE_ASSET)
        {
            if(!IridiumTransaction_CheckAssetUpdateDetails(&entity,vout,details,reason))
            {
                return false;            
            }
        }
        ...
        {
            if(!IridiumTransaction_CheckVariableUpdateDetails(tx,&entity,offset,vout,details,reason))
            {
                return false;            
            }
        }
    ...
    else
    {
        ...
        {
            if(!IridiumTransaction_CheckUpgradeApproval(tx,&entity,offset,vout,details,reason))
            {
                return false;            
            }
        }
        else                                                                    // (Pseudo)stream item
        {
            if(entity.GetEntityType() <= MC_ENT_TYPE_STREAM_MAX)
            {
                ...
                if(!IridiumTransaction_CheckStreamItem(&entity,vout,details,reason))
                {
                    return false;            
                }            
            }
...
bool IridiumTransaction_CheckDestinations(const CScript& script1,
                                             int vout,
                                             CIridiumTxDetails *details,   
                                             string& reason)      
{
...
bool IridiumTransaction_CheckIfLibraryApproval(unsigned char* ptr,
                                                  CIridiumTxDetails *details)
{
    mc_EntityDetails entity;
...
bool IridiumTransaction_ProcessPermissions(const CTransaction& tx,
                                              int offset,  
                                              int vout,
                            ...
                            if(details->vInputDestinations[i] != 0)
                            {
                                if( ( (type & (MC_PTP_ADMIN | MC_PTP_MINE)) == 0) || details->vInputCanGrantAdminMine[i] || (entity.GetEntityType() > 0) )
                                {
                                            ...
                                            from,to,timestamp,flags,1,offset) == 0)
                                    {
                                        if(type & MC_PTP_FILTER)
                                        {
                                            IridiumTransaction_CheckIfLibraryApproval(ptr,details);
                                        }
                                        fAdminFound=true;
                                    }
                                }
...
bool Is_IridiumLicenseTokenTransfer(const CTransaction& tx)
{
    mc_EntityDetails entity;
    if(tx.IsCoinBase())
    {
    ...
    IridiumTransaction_SetTmpOutputScript(tx.vout[0].scriptPubKey);
...
bool IridiumTransaction_CheckLicenseTokenTransfer(const CTransaction& tx,
                                               int unchecked_row, 
                                               CIridiumTxDetails *details,   
                                               string& reason)      
{
...
bool IridiumTransaction_CheckAssetTransfers(const CTransaction& tx,
                                               int offset,  
                                               int vout,
                                               CIridiumTxDetails *details,   
                                               string& reason)      
{
    ...
    if(!IridiumTransaction_CheckLicenseTokenTransfer(tx,unchecked_row,details,reason))
    {
        return false;
    }
...
bool IridiumTransaction_ProcessTokenIssuance(const CTransaction& tx,
                                                int offset,  
                                                bool accept,
                                                int vout,
...
bool IridiumTransaction_CheckOutputs(const CTransaction& tx,                 // Tx to check
                                        const CCoinsViewCache &inputs,          // Tx inputs from UTXO database
                                        ...
                                        string& reason)                         // Error message
{
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)                  // Basic checks, destinations and simple grants
    {
        ...
        IridiumTransaction_SetTmpOutputScript(tx.vout[vout].scriptPubKey);

        if(mc_gState->m_TmpScript->IsOpReturnScript())                    
        {
            if(!IridiumTransaction_CheckOpReturnScript(tx,inputs,vout,details,reason))
            {
                return false;
            }
        }
        ...
        {
            if(!IridiumTransaction_CheckDestinations(tx.vout[vout].scriptPubKey,vout,details,reason))
            {
                return false;
            }            
            ...
            if(!IridiumTransaction_ProcessPermissions(tx,offset,vout,permission_type,true,details,reason))
            {
                return false;
            }            
        }
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)                  // create, issue, activate grants (requiring pre-tx permissions)
    {
        if(details->vOutputScriptFlags[vout] & (MC_MTX_OUTPUT_DETAIL_FLAG_PERMISSION_CREATE | MC_MTX_OUTPUT_DETAIL_FLAG_PERMISSION_FILTER) )
        {
            ...
            if(!IridiumTransaction_ProcessPermissions(tx,offset,vout,permission_type,false,details,reason))
            {
                return false;
            }                        
        }
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)
    {
        ...
        if(details->vOutputScriptFlags[vout] & MC_MTX_OUTPUT_DETAIL_FLAG_OP_RETURN_ENTITY_ITEM)
        {
            ...
            if(!IridiumTransaction_CheckEntityItem(tx,offset,vout,details,reason))
            {
                return false;                
            }
        }                                                                                        
    ...
    if(mc_gState->m_Features->NFTokens())
    {
        for (unsigned int vout = 0; vout < tx.vout.size(); vout++)
        {
            ...
            if(!IridiumTransaction_ProcessTokenIssuance(tx,offset,accept,vout,0,details,reason))
            {
                return false;                
            }        
        }    
...
bool IridiumTransaction_CheckTransfers(const CTransaction& tx,               // Tx to check
                                        int offset,                             // Tx offset in block, -1 if in memppol
                                        CIridiumTxDetails *details,          // Tx details object
                                        string& reason)                         // Error message
{
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)
    {
        ...
        if(details->vOutputScriptFlags[vout] & MC_MTX_OUTPUT_DETAIL_FLAG_NOT_OP_RETURN)
        {
            ...
            if(!IridiumTransaction_CheckAssetTransfers(tx,offset,vout,details,reason))
            {
                return false;                
            }
        }        
...
int64_t IridiumTransaction_OffchainFee(int64_t total_offchain_size)            // Total size of offchain items
{
    return (MIN_OFFCHAIN_FEE*total_offchain_size + 999)/ 1000;
}

bool IridiumTransaction_CheckMandatoryFee(CIridiumTxDetails *details,     // Tx details object
                                             int64_t *mandatory_fee,            // Mandatory Fee    
                                             string& reason)                    // Error message
{
    *mandatory_fee = IridiumTransaction_OffchainFee(details->total_offchain_size);
...
bool IridiumTransaction_CheckLicenseTokenDetails(CIridiumTxDetails *details,     // Tx details object
...
bool IridiumTransaction_ProcessAssetIssuance(const CTransaction& tx,         // Tx to check
                                                int offset,                     // Tx offset in block, -1 if in memppol
            ...
            (details->new_entity_type != MC_ENT_TYPE_LIBRARY) )
        {
            ...
            {                    
                if(mc_gState->m_Features->LicenseTokens())
                {
                    ...
                    IridiumTransaction_SetTmpOutputScript(tx.vout[issue_vout].scriptPubKey);
                    if(mc_gState->m_Features->License20010())
                    {
                        if(mc_gState->m_TmpScript->GetNumElements() < 2)
                        {
                    ...
                    if(!IridiumTransaction_CheckLicenseTokenDetails(details,token_address,reason))
                    {
                        return false;                                                                                                                                                        
                    }
    ...
    if(err == 0)
    {
        if(mc_gState->m_Features->NFTokens())
        {
            if(followon_mode & MC_ENT_FOMD_NON_FUNGIBLE_TOKENS)
            {
                ...
                {
                    IridiumTransaction_SetTmpOutputScript(tx.vout[vout].scriptPubKey);
                    if(!IridiumTransaction_ProcessTokenIssuance(tx,offset,accept,vout,txid,details,reason))
                    {
                        return false;                
                    }        
                }
...
bool IridiumTransaction_ProcessEntityCreation(const CTransaction& tx,        // Tx to check
                                                 int offset,                    // Tx offset in block, -1 if in memppol
                                                 bool accept,                   // Accept to mempools if successful
                                                 CIridiumTxDetails *details, // Tx details object
                                                 string& reason)                // Error message
...
//                if( (details->new_entity_type <= MC_ENT_TYPE_STREAM_MAX) ||     // Admin persmission is required for upgrades and filters
    ...
    if(update_mempool)
    {
        if(mc_gState->m_Assets->FindEntityByTxID(&entity,(unsigned char*)&txid))
        {
            ...
            if(details->new_entity_type == MC_ENT_TYPE_FILTER)
            {
                pIridiumFilterEngine->AddFilter((unsigned char*)&txid+MC_AST_SHORT_TXID_OFFSET,(offset < 0) ? 0 : 1);
            }
        }
        else
        {
...
bool IridiumTransaction_VerifyNotFilteredRestrictions(const CTransaction& tx,        // Tx to check
                                                         int offset,                    // Tx offset in block, -1 if in memppol
                                                         bool accept,                   // Accept to mempools if successful
                                                         CIridiumTxDetails *details, // Tx details object
                                                         string& reason)                // Error message
...
bool IridiumTransaction_VerifyStandardCoinbase(const CTransaction& tx,        // Tx to check
                                                        CIridiumTxDetails *details, // Tx details object
                                                        string& reason)                // Error message
{
    ...
    if(signature_output == (mc_gState->m_NetworkParams->IsProtocolBitchain() == 0))
    {
        ...
        return true;                                                            // Signature output should appear only if protocol=iridium
    }
...
bool IridiumTransaction_ProcessLibraryUpdates(int offset,
                                                 CIridiumTxDetails *details, // Tx details object                                                 
                                                 string& reason)                // Error message
{
    ...
    if(pIridiumFilterEngine)
    {    
        pIridiumFilterEngine->CheckLibraries(&(details->vAffectedLibraries),(offset >= 0));
    }
    return true;
}


bool AcceptIridiumTransaction   (const CTransaction& tx,                     // Tx to check
                                    const CCoinsViewCache &inputs,              // Tx inputs from UTXO database
                                    int offset,                                 // Tx offset in block, -1 if in memppol
                                    ...
                                    uint32_t *replay)                           // Replay flag - if tx should be rechecked or only permissions
{
    ...
    details.fCheckCachedScript=IridiumTransaction_CheckCachedScriptFlag(tx);
    
    mc_gState->m_TmpAssetsIn->Clear();
    
    if(!IridiumTransaction_CheckCoinbaseInputs(tx,&details))                 // Inputs
    {
        if(!IridiumTransaction_CheckInputs(tx,inputs,&details,reason))
        {
            return false;
        }
    }
    ...
    if(!IridiumTransaction_CheckOutputs(tx,inputs,offset,accept,&details,reason))   // Outputs        
    {
        fReject=true;
        goto exitlbl;                                                                    
    }
    
    if(!IridiumTransaction_ProcessAssetIssuance(tx,offset,accept,&details,reason))                // Asset genesis/followon
    {
        fReject=true;
        goto exitlbl;                                                                    
    }

    if(!IridiumTransaction_CheckMandatoryFee(&details,&mandatory_fee,reason))
    {
        fReject=true;
        goto exitlbl;                                                                            
    }                                                                           

    if(!IridiumTransaction_CheckTransfers(tx,offset,&details,reason))        // Transfers and receive permissions
    {
        fReject=true;
        goto exitlbl;                                                                    
    }
                                                                                // Creating of (pseudo)streams/upgrades
    if(!IridiumTransaction_ProcessEntityCreation(tx,offset,accept,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                    
    }

    if(!IridiumTransaction_VerifyNotFilteredRestrictions(tx,offset,accept,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                                
    }        
    
    if(!IridiumTransaction_VerifyStandardCoinbase(tx,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                                
    }        
    
    if(!IridiumTransaction_ProcessLibraryUpdates(offset,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                                
    }        
        ...
        ((flags & MC_AMT_NO_FILTERS) == 0) )
    {
        if(mc_gState->m_Features->Filters())
        {
            if(pIridiumFilterEngine)
            {
                mc_IridiumFilter* lpFilter;
                int applied=0;

                if(pIridiumFilterEngine->RunTxFilters(tx,details.vRelevantEntities,reason,&lpFilter,&applied,(offset >= 0)) != MC_ERR_NOERROR)
                {
...
exitlbl:
    ...
    if(!accept || fReject)                                                      // Rolling back permission database if we were just checking or error occurred    
    {
        ...
        IridiumTransaction_ProcessLibraryUpdates(offset,&details,reason);
    }
