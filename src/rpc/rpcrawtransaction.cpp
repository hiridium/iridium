 ...
 */ 
    ...
    {
//        if(pIridiumFilterEngine->m_TxID != 0)
        if(pIridiumFilterEngine->InFilter())
        {
...
/* MCHN START */    
// TODO too many duplicate code with ListWalletTransactions and may be AccepIridiumTransaction
...
int VerifyNewTxForStreamFilters(const CTransaction& tx,std::string &strResult,mc_IridiumFilter **lppFilter,int *applied)            
{
    ...
    if(pIridiumFilterEngine->NoStreamFilters())
    {
        return MC_ERR_NOERROR;
    }
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++) 
    {
        ...
        if(result.type() == obj_type)
        {
            ...
            int err=pIridiumFilterEngine->RunStreamFilters(tx,i,(unsigned char*)&hash+MC_AST_SHORT_TXID_OFFSET,-1,0, 
...
//        throw runtime_error("Help message not found\n");
    ...
    if(pIridiumFilterEngine->m_Vout < 0)
    {
    ...
    if(pIridiumFilterEngine->m_Vout >= (int)pIridiumFilterEngine->m_Tx.vout.size())
    {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "vout out of range");                                            
    }
    Value result=DataItemEntry(pIridiumFilterEngine->m_Tx,pIridiumFilterEngine->m_Vout,streams_already_seen, 0x0E00);
...
//    result.get_obj().push_back(Pair("txid", pIridiumFilterEngine->m_Tx.GetHash().GetHex()));
    result.get_obj().push_back(Pair("vout", pIridiumFilterEngine->m_Vout));
...
Value getfilterstream(const Array& params, bool fHelp)
{
    ...
    Value result=StreamEntry((unsigned char*)&(pIridiumFilterEngine->m_EntityTxID),0x0002);
...
*/    
    Object result;
    TxToJSON(pIridiumFilterEngine->m_Tx, 0, result);
...
Value disablerawtransaction(const Array& params, bool fHelp)
{
    ...
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "disablerawtransaction supported only for protocol=iridium");
    }    
