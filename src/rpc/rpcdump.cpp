...
Value importaddress(const Array& params, bool fHelp)
{
    ...
    for(int is=0;is<(int)inputStrings.size();is++)
    {
        ...
        if (address.IsValid()) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Iridium address or script");
        }
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    ...
    if (!address.SetString(strAddress))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Iridium address");
    CKeyID keyID;
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by Iridium %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
