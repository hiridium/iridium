...
The following instructions fetch the Google V8 JavaScript engine to your local machine and configure it to create static libraries in the location where the Iridium build system expects to find them.

Iridium uses V8 version 6.8, and requires at least 4 GB of RAM to build in a reasonable time. It will not build at all with less than 2 GB RAM.
...
The following commands check out V8 and select the branch used by Iridium. Please note that this step downloads about 2 GB of data, and can take a long time (30 minutes or more).
...
Create an additional library embedding the V8 initial snapshot blobs:
    ...
    cd $RELEASE
    python $IRIDIUM_HOME/depends/v8_data_lib.py
    
### Note

The environment variable `$IRIDIUM_HOME` should point to the parent of the V8 build folder (see [mac.md](mac.md) for details).
